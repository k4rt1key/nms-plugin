...
type Result struct {
	MetricGroup schema.MetricGroup
	Success     bool
	Data        string
	Message     string
	Time        time.Time
...
func Execute(request schema.PollingRequest) schema.PollingResponse {
	logger.Info("Starting polling process for %d metric groups", len(request.MetricGroups))

	// Create the response object
	response := schema.PollingResponse{
		Type:         schema.PollingType,
		MetricGroups: make([]schema.PollingResult, 0, len(request.MetricGroups)),
	}
		...
		go func(mg schema.MetricGroup) {
			...
			if err != nil {
				...
				logger.Error("Failed to get metrics for group %s (ID: %d) on %s: %v",
					mg.Name, mg.MonitorID, mg.IP, err)
			} else {
				result.Success = true
				result.Data = data
	...
	for result := range resultsChan {
		pollingResult := schema.PollingResult{
