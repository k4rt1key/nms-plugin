...
func main() {
	...
	var requestType struct {
		Type models.RequestType `json:"type"`
	}
	...
	case models.DiscoveryType:
		response = handleDiscovery(input)
	case models.PollingType:
...
func handleDiscovery(input string) models.DiscoveryResponse {
	var request models.DiscoveryRequest
...
func handlePolling(input string) models.PollingResponse {
	var request models.PollingRequest
	if err := json.Unmarshal([]byte(input), &request); err != nil {
